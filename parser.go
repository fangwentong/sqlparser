...
import (
	...
	"github.com/xwb1989/sqlparser/internal/dependency/vt/servenv"
	vterrors2 "github.com/xwb1989/sqlparser/internal/dependency/vt/vterrors"
	"io"
	"strconv"
	"strings"
	"sync"
...
func Parse2(sql string) (Statement, BindVars, error) {
	...
	if yyParsePooled(tokenizer) != 0 {
		...
		return nil, nil, vterrors2.New(vtrpcpb.Code_INVALID_ARGUMENT, tokenizer.LastError.Error())
	}
...
func convertMySQLVersionToCommentVersion(version string) (string, error) {
	...
	if idx == 0 {
		return "", vterrors2.Errorf(vtrpcpb.Code_INVALID_ARGUMENT, "MySQL version not correctly setup - %s.", version)
	}
...
var ErrEmpty = vterrors2.NewErrorf(vtrpcpb.Code_INVALID_ARGUMENT, vterrors2.EmptyQuery, "query was empty")
